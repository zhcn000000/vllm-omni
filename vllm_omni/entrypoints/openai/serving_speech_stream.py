...
class OmniStreamingSpeechHandler:
    ...
    async def handle_session(self, websocket: WebSocket, raw_request: Request | None = None) -> None:
            ...
            while True:
                ...
                if msg_type == "input.text":
                    ...
                    for sentence in sentences:
                        await self._generate_and_send(websocket, raw_request, config, sentence, sentence_index)
                        sentence_index += 1
                    ...
                    if remaining:
                        await self._generate_and_send(websocket, raw_request, config, remaining, sentence_index)
                        sentence_index += 1
    ...
    async def _generate_and_send(
        ...
        request_id = f"speech_{self._speech_service._base_request_id(raw_request)}"
        try:
            if config.stream_audio:
                ...
                audio_bytes, _ = await self._speech_service._generate_audio_bytes(request, request_id)
